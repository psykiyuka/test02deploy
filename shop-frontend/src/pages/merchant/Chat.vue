      ...
      <template v-else>
        ...
        <div class="chat-header">
          ...
          <span v-if="currentSession?.product_name" class="chat-product-tag">
            商品：{{ currentSession.product_name }}
          </span>
        </div>
...
async function loadSessions() {
  try {
    const res: any = await api.get('/m-endpoint/chat/sessions')
...
async function selectSession(s: any) {
  ...
  // 标记已读
  try {
    await api.post('/m-endpoint/chat/mark-read', null, {
...
async function loadMessages() {
  ...
  try {
    const res: any = await api.get('/m-endpoint/chat/messages', {
...
// 增量轮询：只拉取 after_id 之后的新消息，不全量覆盖
...
async function sendReply() {
  ...
  try {
    const res: any = await api.post('/m-endpoint/chat/reply', {
      session_id: currentSessionId.value,
      message: text,
    })
    if (res.code === 0) {
      replyText.value = ''
      // 本地追加消息，带上服务端返回的 id（用于增量轮询 after_id）
...
.chat-msg.merchant { align-items: flex-end; }
.chat-msg-name {
  font-size: 12px;
  color: #999;
  margin-bottom: 4px;
...
.chat-msg.merchant .chat-msg-name { text-align: right; }
.chat-msg-bubble {
  max-width: 70%;
  padding: 10px 16px;
  border-radius: 16px;
...
.chat-msg.merchant .chat-msg-bubble {
  background: linear-gradient(135deg, #6366f1, #8b5cf6);
  color: #fff;
